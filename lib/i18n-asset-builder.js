...
I18NAssetBuilder.prototype.build = function() {
  ...
  var prefixOtherLangFiles = this.options.prefixOtherLangFiles;
  var defaultLocale = this.options.defaultLocale;
  var defaultLocaleFile = path.join(inputPath, defaultLocale + '.properties');
  ...
  fs.readdirSync(inputPath, { encoding: 'utf8' }).forEach(function(file) {
    var baseFileName = path.basename(file, '.properties');
    var fileName = (!prefixOtherLangFiles && baseFileName !== defaultLocale) ? baseFileName : outputFilePrefix + baseFileName;
